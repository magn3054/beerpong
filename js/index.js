...
function generateTournament() {
    ...
    rounds.reverse().forEach((roundMatches, index) => {
        ...
        roundDiv.className = 'round round-' + (totalRounds - index);
    ...
    // Initialize nextRoundSpots for the tournament
    nextRoundSpots = {};
    for (let i = 1; i < totalRounds; i++) {
        nextRoundSpots[i + 1] = 1;
    }
}
...
function selectWinner(teamDiv, team1, team2, currentRoundIndex, matchIndex, side) {
    ...
    if (nextRoundIndex >= 0) {
        const nextSpotIndex = nextRoundSpots[nextRoundIndex];
        if (nextSpotIndex !== undefined) {
            const nextWinnerId = `${nextRoundIndex}-${nextSpotIndex}`;
            const nextWinnerDiv = document.getElementById(nextWinnerId);
            
            if (nextWinnerDiv) {
                nextWinnerDiv.textContent = teamDiv.textContent;
                nextRoundSpots[nextRoundIndex]++;
            }
        }
    }
